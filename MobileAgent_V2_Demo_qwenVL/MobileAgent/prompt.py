def get_action_prompt(instruction, clickable_infos, width, height, keyboard, summary_history, action_history, last_summary, last_action, add_info, error_flag, completed_content, memory):
    ...
    prompt += "Now you need to combine all of the above to perform just one action on the current page. You must choose one of the following actions:\n"
    prompt += "Open app (app name): If the current page is desktop, use this action to open the app named \"app name\" on the desktop.\n"
    prompt += "Tap (x, y): Tap the position (x, y) on the current page.\n"
    prompt += "Swipe (x1, y1), (x2, y2): Swipe from position (x1, y1) to position (x2, y2).\n"
    prompt += "LongTap (x, y, t): Tap the position (x, y) for t seconds on the current page.\n"
    prompt += "Tap scale (x1, y1, x2, y2): Touch at position (x1, y1) and lift at position (x2, y2) on the current page.\n"
    prompt += "DoubleTap (x, y): Perform two quick taps at position (x, y) on the current page.\n"
    prompt += "Search (Tap + Type): Tap at a specified position, then type the input on the current page.\n"
    prompt += "Drag (x1, y1), (x2, y2): Tap and hold at position (x1, y1), then drag to position (x2, y2).\n"
    prompt += "Pinch (x1, y1, x2, y2, scale): Pinch at the specified positions with a scaling factor on the current page.\n"
    prompt += "Rotate (x1, y1, x2, y2, angle): Rotate at the specified positions by a certain angle.\n"
    prompt += "Note: You may need to combine atomic actions like Tap, LongTap, and Swipe to create complex actions such as DoubleTap or Drag. Similarly, you can perform a Search by combining Tap with typing input (Tap + Type).\n"
    if keyboard:
