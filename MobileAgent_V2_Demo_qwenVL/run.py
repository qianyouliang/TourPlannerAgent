...
adb_path = "D:/AndroidStudtioProject/MobileAgent/MobileAgent_V2_Demo_qwenVL/platform-tools/adb.exe"

# Your instruction
instruction = "Read the Screen, Tap feizhulvxing APP, then tap the hotel icon and search for a hotel in Beijing. After that, switch to the next app, like 日历."
...
ocr_detection = pipeline(Tasks.ocr_detection, model='iic/cv_resnet18_ocr-detection-line-level_damo', device="gpu")
ocr_recognition = pipeline(Tasks.ocr_recognition, model='iic/cv_convnextTiny_ocr-recognition-document_damo', device="gpu")
